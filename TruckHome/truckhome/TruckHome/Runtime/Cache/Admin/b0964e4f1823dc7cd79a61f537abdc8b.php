						...
						<table class="table table-striped table-bordered table-hover ">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
										...
										<td style="text-indent: <?php echo ($vo['level']*30); ?>px">
											<?php if($vo['pid'] == 0): ?><img src="/threeall/ThinkPHP/Public/shop/admin/images/dirfirst.gif">
											<?php else: ?>
												<img src="/threeall/ThinkPHP/Public/shop/admin/images/dirsecond.gif"><?php endif; ?>
											&nbsp;&nbsp;<?php echo ($vo['catename']); ?>
										</td>
										<td>
											正在<?php echo ($state[!$vo['status']]); ?>
										</td>
										<td>
											<a href="<?php echo U('Scategory/add',array('id'=>$vo['id'],'name'=>$vo['name']));?>" class="btn btn-warning btn-xs ">添加子类</a>
											<a href="<?php echo U('Scategory/edit',array('id'=>$vo['id']));?>" class="btn btn-warning btn-xs ">修改</a>
											<a href="<?php echo U('Scategory/status',array('id'=>$vo['id']));?>" class="btn btn-danger btn-xs ">
							...
							<tr>
								<td colspan="6" class="text-right"><?php echo ($show); ?></td>
							</tr>
						</table>
					</div>
					<!-- /.table-responsive -->
