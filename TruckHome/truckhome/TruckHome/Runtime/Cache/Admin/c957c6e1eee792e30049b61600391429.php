	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">修改品牌类别</div>
				<div class="panel-body">
					<div class="row">
						<div class="col-lg-12">
							<form class="form-horizontal" action="<?php echo U('Xcartype/update');?>" method="post">


								<div class="form-group">
									<label for="inputPassword3" class="col-sm-2 control-label">车型分类:</label>
									<div class="col-sm-10">
										<?php if($data['pid'] == 0): ?><p>根目录</p>
										<?php else: ?>
											<select style="width:50%" class="form-control" name="pid">
											<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php echo ($vo['id']==$data['pid']?"selected":""); ?> style="padding-left:<?php echo ($vo['level']*50); ?>px;">
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input type="text" class="form-control" value="<?php echo ($data['name']); ?>" style="width:50%" id="name" placeholder="标题" name="name">
										<input type="hidden" class="form-control" value="<?php echo ($data['id']); ?>" style="width:50%" id="id" placeholder="" name="id">
									</div>
								</div>
								<?php if($data['pid'] == 0): else: ?>

									<div class="form-group">
									<label for="inputPassword3" class="col-sm-2 control-label">品牌:</label>
									<div class="col-sm-10">

										<select style="width:50%" class="form-control" name="bid">
											<?php if(is_array($brandlist)): $i = 0; $__LIST__ = $brandlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bo): $mod = ($i % 2 );++$i; if( in_array($bo['id'],$ppid)): ?><option value="<?php echo ($bo['id']); ?>" disabled <?php echo ($bo['id']==$data['bid']?"selected":""); ?> >
													<?php echo str_repeat("&nbsp;",$bo['level']*5);?>|--<?php echo ($bo['name']); ?>
												</option>
												<?php else: ?>
													<option value="<?php echo ($bo['id']); ?>"  <?php echo ($bo['id']==$data['bid']?"selected":""); ?> >
													<?php echo str_repeat("&nbsp;",$bo['level']*5);?>|--<?php echo ($bo['name']); ?>
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										...
										<label class="radio-inline">
											<input type="radio" name="tag" id="sex" value="1" <?php echo ($data['tag']==1?"checked":""); ?>/>热门卡车
										</label>
										<label class="radio-inline">
											<input type="radio" name="tag" id="sex" value="2" <?php echo ($data['tag']==2?"checked":""); ?>/>挂车 专用车  皮卡
										</label>
										<label class="radio-inline">
											<input type="radio" name="tag" id="sex" value="3" <?php echo ($data['tag']==3?"checked":""); ?>/>天然气车型
										</label>
									</div>
								</div>
								<div class="form-group">
